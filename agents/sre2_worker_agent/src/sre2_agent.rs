...
use core_logic::{
    agents::*, constants::*, discord_notifier, dispatcher::*, envs_manage, errors::AppError,
    file_reader, machine::*, prompts::*, schemas::*, write_debug_log::*,
};
...
pub async fn run_read(message_transmitted: String) -> Sre2AgentNodeResult<LlmResponse> {
    ...
    if model.trim().is_empty() {
        return Err(AppError::Env(
            "Model env var is null error, make sure to select a model to make any API call."
                .to_string(),
        ));
    }
    ...
    println!(
        "Final Answer from Sre2 Agent `Read` Agent: {}",
        final_answer
    );

    // we format the new prompt adding the schema with our helper function coming `schema.rs`
    let string_schema =
        get_schema_fields(&core_logic::schemas::sre2_agent_own_task_read_files_schema());
    let final_answer_plus_string_schema = format!(
        "{}. {}.",
        final_answer.choices[0].message.content.clone().ok_or(
            AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (run_read)".to_string()
            )
        )?, // result form tool call
        string_schema,
    );
...
pub async fn run_write(message_transmitted: String) -> Sre2AgentNodeResult<LlmResponse> {
    ...
    .await?;
    println!(
        "Final Answer from Sre2 Agent `Write` Agent: {}",
        final_answer
    );

    // we format the new prompt adding the schema with our helper function coming `schema.rs`
    let string_schema =
        get_schema_fields(&core_logic::schemas::sre2_agent_own_task_write_files_schema());
    let final_answer_plus_string_schema = format!(
        "{}. {}.",
        final_answer.choices[0].message.content.clone().ok_or(
            AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (run_write)".to_string()
            )
        )?, // result form tool call
        string_schema,
    );
...
pub async fn run_commit(message_transmitted: String) -> Sre2AgentNodeResult<LlmResponse> {
    ...
    // we format the new prompt adding the schema with our helper function coming `schema.rs`
    let string_schema =
        get_schema_fields(&core_logic::schemas::sre2_agent_own_task_commit_schema());
    let final_answer_plus_string_schema = format!(
        "{}. {}.",
        final_answer.choices[0].message.content.clone().ok_or(
            AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (run_commit)".to_string()
            )
        )?, // result form tool call
        string_schema,
    );
...
pub async fn run_report(state: StateReportSreToPr) -> Sre2AgentNodeResult<LlmResponse> {
    ...
    // we format the new prompt adding the schema with our helper function coming `schema.rs`
    let string_schema = get_schema_fields(&core_logic::schemas::sre2_agent_to_pr_agent_schema());
    let final_answer_plus_string_schema = format!(
        "{}. {}.",
        final_answer.choices[0].message.content.clone().ok_or(
            AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (run_report)".to_string()
            )
        )?, // result form tool call
        string_schema,
    );
...
pub async fn sre2_agent_node_work_orchestration(
    message_transmitted: String,
    ...
    let read_output_schema =
        read.choices[0]
            .message
            .content
            .clone()
            .ok_or(AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (sre2_agent_node_work_orchestration: run_read)"
                    .to_string(),
            ))?;
    ...
    let read_output_transmitted_formatted = format!(
        "manifest with instructions in what to modify and the file path to write to: {}",
        read_output_to_value
    );
    let read_initial_manifest = match read_output_to_value
        .get("manifest")
        .and_then(|v| v.as_str())
    {
        Some(s) => s.trim(),
    ...
    let write_output_schema =
        write.choices[0]
            .message
            .content
            .clone()
            .ok_or(AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (sre2_agent_node_work_orchestration: run_write)"
                    .to_string(),
            ))?;
    let write_output_to_value: Value = serde_json::from_str(&write_output_schema)?;
    let write_output_transmitted_formatted = format!(
        "new manifest and path of the file modified follwoing instructions: {}",
        write_output_to_value
    );
    let write_final_manifest = match write_output_to_value
        .get("manifest")
        .and_then(|v| v.as_str())
    {
        Some(s) => s.trim(),
    ...
    // logs of write mini agent output
    write_step_cmd_debug("\nWRITE:\n");
    write_step_cmd_debug(&write_output_schema);

    // we can check if path are the same and if not we exit/fail early
    println!(
        "read file path: {}\nwrite file path: {}",
        read_initial_manifest_path, write_final_manifest_path
    );
    if read_initial_manifest_path != write_final_manifest_path {
        return Err(AppError::Sre2AgentNode(
            "Initial file path red different from final file path written to.".to_string(),
        ));
    }

    // then we commit
    let commit = run_commit(write_output_transmitted_formatted.clone()).await?; // Result<LlmResponse>
    let commit_output_schema =
        commit.choices[0]
            .message
            .content
            .clone()
            .ok_or(AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (sre2_agent_node_work_orchestration:run_commit)"
                    .to_string(),
            ))?;
    ...
    let commit_message = match commit_output_to_value
        .get("message")
        .and_then(|v| v.as_str())
    {
        Some(s) => s.trim(),
    ...
    let report_output_schema =
        report.choices[0]
            .message
            .content
            .clone()
            .ok_or(AppError::StructureFinalOutputFromRaw(
                "couldn't parse final answer (sre2_agent_node_work_orchestration: run_report)"
                    .to_string(),
            ))?;
    ...
    let report_output_transmitted_formatted = format!(
        ...
        message_transmitted, report_output_to_value
    );
