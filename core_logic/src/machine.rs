...
use crate::agents::*;
use crate::constants;
use crate::{errors::AppError, headers::get_auth_headers};
use reqwest::{
    Client,
...
pub fn machine_prompt(role: &UserType, content: &str) -> PromptMachineResult<MessagesSent> {
    // done!
    Ok(MessagesSent::create_new_message_struct_to_send(
        role, content,
    ))
}
...
pub fn machine_create_payload_with_or_without_tools_structout(
    ...
    match Payload::create_payload_with_or_without_tools_structout(
        ...
        Err(e) => Err(AppError::Payload(format!(
            "An error occured while trying to create the payload to send: {}",
            e
        ))),
    }
}
...
//   Ok(llm_response)
// }
pub async fn machine_api_call(endpoint: &str, payload: &Value) -> CallApiResult<LlmResponse> {
    let client = Client::new();
    ...
    let body = response
        .text()
        .await
        .map_err(|e| AppError::Agent(format!("Failed to read response text: {}", e)))?;
    ...
    match tool_name {
            ...
            let file_path = parsed_args
                ...
                .ok_or_else(|| {
                    AppError::ExecuteToolEngine(
                        "Missing `file_path` argument for `read_file_tool`".into(),
                    )
                })?;
        ...
        "write_file_tool" => {
            let file_path = parsed_args
                ...
                .ok_or_else(|| {
                    AppError::ExecuteToolEngine(
                        "Missing `file_path` argument for `write_file_tool`".into(),
                    )
                })?;
            let file_content = parsed_args
                .get("file_content")
                .and_then(|v| v.as_str())
                .ok_or_else(|| {
                    AppError::ExecuteToolEngine(
                        "Missing `file_content` argument for `write_file_tool`".into(),
                    )
                })?;

            let final_file_content = constants::write_file_tool(file_path, file_content); // it is returning a `String`
...
pub fn engine_prompt(
    role: &UserType,
    content: &str,
) -> PromptEngineResult<HashMap<String, String>> {
...
pub fn get_specific_agent_schema_engine(
    full_struct_out: &StructOut,
    agent_role: &AgentRole,
) -> GetSchemaEngineResult<Schema> {
    if let Some(target_schema) = full_struct_out.get_by_role(agent_role) {
        Ok(target_schema.clone())
    } else {
        Err(AppError::GetSchemaEngine(
            "get specific agent schema error".to_string(),
        ))
    }
}
...
pub fn get_prompt_user_and_content_engine(
    prompt: &HashMap<UserType, &str>,
) -> GetPromptUserAndContentEngineResult<(UserType, String)> {
...
pub fn messages_format_engine(
    type_user: &UserType,
    content: &str,
) -> MessagesFormatEngineResult<HashMap<String, String>> {
...
pub fn create_agent_engine(
    ...
    let new_agent = machine_agent(role, message, prompt, struct_out, task_state, llm_settings)?;
    Ok(new_agent)
}
...
/* ** PAYLOAD ENGINE  ** */
 // MIGHT NOT BE NEEDED AS API CALL CALL LOOP FUNCTION CREATES IT ON THE FLY
